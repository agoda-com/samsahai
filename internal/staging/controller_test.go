...
var _ = Describe("Apply Env Based Config", func() {
	...
	It("Should successfully apply configuration based on queue type", func() {
...

		{
			...
			values = applyEnvBaseConfig(cfg, values, s2hv1beta1.QueueTypeUpgrade, comps["redis"])
		...
		{
			values := util.CopyMap(comps["redis"].Values)
			values = applyEnvBaseConfig(cfg, values, s2hv1beta1.QueueTypePreActive, comps["redis"])
			v, err := dotaccess.Get(values, "master.service.nodePort")
			g.Expect(err).NotTo(HaveOccurred())
			port, ok := v.(float64)
		...
		{
			values := util.CopyMap(comps["redis"].Values)
			values = applyEnvBaseConfig(cfg, values, s2hv1beta1.QueueTypePromoteToActive, comps["redis"])
			v, err := dotaccess.Get(values, "master.service.nodePort")
			g.Expect(err).NotTo(HaveOccurred())
			port, ok := v.(float64)
		...
		{
			values := util.CopyMap(comps["redis"].Values)
			values = applyEnvBaseConfig(cfg, values, s2hv1beta1.QueueTypeDemoteFromActive, comps["redis"])
	...
	It("Should correctly combine base values and config", func() {
		...
		envValues, err := configctrl.GetEnvComponentValues(cfg, "wordpress", s2hv1beta1.EnvBase)
		g.Expect(err).NotTo(HaveOccurred())
