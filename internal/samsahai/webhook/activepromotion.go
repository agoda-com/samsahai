...
type activePromotion struct {
	// +Optional
	Running []s2hv1.ActivePromotion `json:"running"`
...
func (h *handler) getActivePromotions(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	...
	runningList := make([]s2hv1.ActivePromotion, 0)
	waitingList := make([]string, 0)
	if len(atpList.Items) > 0 {
		for _, atp := range atpList.Items {
			if atp.Status.State == s2hv1.ActivePromotionWaiting {
...
type teamActivePromotion struct {
	// +Optional
	Current *s2hv1.ActivePromotion `json:"current"`
...
type activePromotionHistories []s2hv1.ActivePromotionHistory
...
func (h *handler) getActivePromotionHistoryListByDESC(teamName string) (*s2hv1.ActivePromotionHistoryList, error) {
