...
const commitStatusAPI = "%s/repos/%s/statuses/%s" // base login url, repository, commit SHA
...
type Github interface {
	...
	PublishCommitStatus(commitSHA, targetURL, description, context string, status CommitStatus) error
}
...
type Client struct {
	repository string // e.g., agoda-com/samsahai
	token      string
...
func NewClient(baseURL, repository, token string) *Client {
	client := &Client{
		baseURL:    baseURL,
...
// PublishCommitStatus publishes a commit status for a given SHA
func (c *Client) PublishCommitStatus(commitSHA, labelName, targetURL, description string, status CommitStatus) error {
	logger.Debug("committing a status check",
		"repository", c.repository, "commitSHA", commitSHA, "status", status)

	commitStatusAPI := fmt.Sprintf(commitStatusAPI, c.baseURL, c.repository, commitSHA)
	...
	case <-ctx.Done():
		logger.Error(s2herrors.ErrRequestTimeout,
			fmt.Sprintf("publishing commit status to github repository: %s, commitSHA: %s took longer than %v",
				c.repository, commitSHA, requestTimeout))
		return s2herrors.ErrRequestTimeout
	case err := <-errCh:
		logger.Error(err, "cannot publish commit status",
			"repository", c.repository, "commitSHA", commitSHA)
		return err
	case <-resCh:
