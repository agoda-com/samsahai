...
`))
		...
		msTeamsClient = NewClient(tenantID, clientID, clientSecret, user, pass, WithBaseURL(server.URL, server.URL))
		err := msTeamsClient.PostMessage("groupID", "channelID", "Hi there!",
			WithAccessToken("token-123456"))
		g.Expect(err).NotTo(HaveOccurred())
	})
...
`))
		...
		It("Should not get channel id due to not found", func(done Done) {
			defer close(done)
			server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				defer GinkgoRecover()
				_, err := ioutil.ReadAll(r.Body)
...
`))
			...
			msTeamsClient = NewClient(tenantID, clientID, clientSecret, user, pass, WithBaseURL(server.URL, server.URL))
			_, err := msTeamsClient.GetChannelID("groupID", "Channel 3")
			g.Expect(err).To(HaveOccurred())
		})
	})
